...
var (
	...
	ErrCantUpdateUser     = errors.New("cannot add the product to cart")
...
func BuyItemFromCart(ctx context.Context, userCollection *mongo.Collection, userID string) error {
	...
	grouping := bson.D{{Key: "$group", Value: bson.D{primitive.E{Key: "_id", Value: "$_id"}, {Key: "total", Value: bson.D{primitive.E{Key: "$sum", Value: "$usercartprice."}}}}}}
	...
	for _, user_item := range getusercart {
		...
		err = userCollection.FindOne(ctx, bson.D{primitive.E{Key: "_id", Value: id}}).Decode(&getcaritems)
		if err != nil {
			log.Println(err)
	...
	filters := bson.D{primitive.E{Key: "_id", Value: id}}
	updates := bson.M{"$push": bson.M{"orders.$[].order_list": bson.M{"$each": getcaritems.UserCart}}}
	_, err = userCollection.UpdateOne(ctx, filters, updates)
	if err != nil {
		log.Println(err)
